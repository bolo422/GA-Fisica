...
class ExemploJuntas : public Test //você cria a sua classe derivada da classe base Test
{
public:
	ExemploJuntas() {
	...
	void Step(Settings& settings) override
	{
		...
		//Para poder desenhar o ponto referente ao local onde est? se aplicando uma for?a
	...
	void Keyboard(int key) override  //esse cabe?alho n?o mudam! :)
	{
		...
		case GLFW_KEY_1:
		{
			...
			body1->SetType(b2_staticBody); //para ficar est?tico
...
static int testIndex = RegisterTest("Examples", "ExemploJuntas", ExemploJuntas::Create);
